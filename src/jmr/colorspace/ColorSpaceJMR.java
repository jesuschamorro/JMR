...
public abstract class ColorSpaceJMR extends ColorSpace {
	...
	*/
	...
	public static ColorSpace getInstance(int ColorSpaceType) {
		...
		default:
			return ColorSpaceJMR.getInstance(ColorSpaceType);
		}
	}
...
//	abstract public float[] toRGB(float[] arg0);
	...
	static void RGB2XYZ(float[] RGB, float[] XYZ) {
		...
		XYZ[0] =  0.436052025f*RGB[0] + 0.385081593f*RGB[1] + 0.143087414f *RGB[2];
		XYZ[1] =  0.222491598f*RGB[0] + 0.71688606f *RGB[1] + 0.060621486f *RGB[2];
		XYZ[2] =  0.013929122f*RGB[0] + 0.097097002f*RGB[1] + 0.71418547f  *RGB[2];
	...
	public String getName(int idx) {
		...
		if( idx < numCmp) return "Cmp"+idx;
		else if(idx == numCmp) return "Alpha";
		else return "Unknown";
	}
	...
	public static String getName(ColorSpace cS,int idx) {
		if(cS instanceof ColorSpaceJMR) return cS.getName(idx);
		else {
	...
	public String toString() {
		...
		for(int i=0;i<this.getNumComponents();i++) {
			str+=this.getName(i)+"=["+this.getMinValue(i)+","+this.getMaxValue(i)+"];";
		}
		str+=")";
		return str;
	}
