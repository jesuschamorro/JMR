...
import jmr.initial.descriptor.MediaDescriptor;
...
public class MPEG7EdgeHistogram extends MPEG7TextureDescriptor {
   ...
   * <li>{@link #threshold} = {@link #DEFAULT_THRESHOLD}</li>
   * <li>{@link #numBlock}= {@link #DEFAULT_NUM_BLOCKS}</li>
   * </ul></p>
   */
  ...
  public MPEG7EdgeHistogram(int threshold, int numBlock) {
    super(MediaDescriptor.TYPE_MPEG7_EHD_DESCRIPTOR, true);
  ...
  /** Compare this <code>MPEG7EdgeHistogram</code> obtect with the
   * <code>MPEG7EdgeHistogram</code> given by parameter
   * <p> This method is valid only for <code>MPEG7EdgeHistogram</code>
   *  media descriptors
   * @param mediaDescriptor MediaDescriptor object to be compared
   * @see #compare(MPEG7EdgeHistogram desc)
   * @return The difference between descriptors
   */
  public JMRResult compare(MediaDescriptor mediaDescriptor) {
    // Only MPEG7EdgeHistogram objects can be compared
    if (! (mediaDescriptor instanceof MPEG7EdgeHistogram)) {
      return (null);
    }
    return (compare( (MPEG7EdgeHistogram) mediaDescriptor));
  }
   ...
   * @param media The image from which the descriptor is calculated
   */
